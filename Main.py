'''note: game is missing some audio sections cause of weird audio bus channel error. This section is not included here
    Also only the 'GO' button is functional
    The code is not the cleanest, nor the most efficient, i tried
    Got borred of doing 2D, so i abandoned this project, sowwy'''
...
#████████████████████████████ \/ VARIABLES \/ ████████████████████████████ \/ VARIABLES \/ ████████████████████████████ \/ VARIABLES \/ ████████████████████████████
# core vars
Game, Shop, Pause, Menu = False, False, False, True

#--------------GAME--------------GAME--------------  
    #game start
game_start_AUD = pygame.mixer.Sound('audio/game start.mp3')
game_start_AUD.set_volume(0.15)
    # audio
game_music_list = []
...
game_music_list[0].set_volume(0.2); game_music_list[1].set_volume(0.1); game_music_list[2].set_volume(0.12); game_music_list[3].set_volume(0.16);
...
gameOverIMG = pygame.image.load('images/game/game over.png').convert_alpha()
...
#--------------SHOP--------------  
shopScreenIMG = pygame.image.load('images/shop/screen.png').convert_alpha()

#--------------PAUSE--------------  
pauseScreenIMG = pygame.image.load('images/pause/screen.png').convert_alpha()

planetTime_0 = pygame.image.load('images/game/planet time 0.png').convert_alpha()
planetTime_1 = pygame.image.load('images/game/planet time 1.png').convert_alpha()
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    ...
                    if PlanetScore.rect.collidepoint(mousePos) and PlanetScore.ready_for_click:
                        score += 10
                        print('planet score boost clicked')
                    ...
                    if PlanetTime.rect.collidepoint(mousePos) and PlanetTime.ready_for_click:
                        ...
                        print('planet slow down clicked')
...
#score update
    ...
    #obsticles update
        ...
        if not PlayerHitbox.Alive:
            screen.blit(gameOverIMG, (250,300))
    #sky objects
...
#████████████████████████████ \/ MENU loop \/ ████████████████████████████ \/ MENU loop \/ ████████████████████████████ \/ MENU loop \/ ████████████████████████████
    ...
    while Menu:
        ...
        if menuImgAlreadyChosen == False: # makes the menu screen image always different after reentering lobby
